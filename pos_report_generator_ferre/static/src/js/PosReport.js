...
export class PosReport extends Component {
    ...
    print_pdf(e) {
        ...
        this.orm.call('pos.report', 'pos_report', [[self.wizard_id]]).then((data) => {
            var action = {
                ...
                'report_name': 'pos_report_generator_ferre.pos_order_report',
                'report_file': 'pos_report_generator_ferre.pos_order_report',
                'data': {
                    'report_data': data
                },
                'context': {
    ...
    apply_filter() {
        ...
        if (this.root.el.querySelector('.order_state')) {
    const _st = this.root.el.querySelector('.order_state').value;
    filter_data_selected.order_state = _st ? _st : false; // write False to clear on 'Cualquiera'
}
