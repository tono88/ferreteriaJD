...
class MultipleInvoiceLayout(models.TransientModel):
        ...
        for wizard in self:
            if wizard.company_id:
                ...
                wizard.preview = ir_ui_view._render_template(
                    'base_accounting_kit_ferre.multiple_invoice_wizard_preview',
                    {'company': wizard.company_id, 'preview_css': preview_css,
                     ...
                     'mi': self.env.ref(
                         'base_accounting_kit_ferre.multiple_invoice_sample_name')
                     })
            else:
    ...
    def _get_layout_for_preview(self):
        ...
        if self.layout == 'web.external_layout_boxed':
            new_layout = 'base_accounting_kit_ferre.boxed'

        elif self.layout == 'web.external_layout_bold':
            new_layout = 'base_accounting_kit_ferre.bold'

        elif self.layout == 'web.external_layout_striped':
            new_layout = 'base_accounting_kit_ferre.striped'

        else:
            new_layout = 'base_accounting_kit_ferre.standard'
