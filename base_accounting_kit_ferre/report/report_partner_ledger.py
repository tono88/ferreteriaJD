...
class ReportPartnerLedger(models.AbstractModel):
    _name = 'report.base_accounting_kit_ferre.report_partnerledger'
    ...
    def _sum_partner(self, data, partner, field):
        ...
        return (contemp and contemp[0]) or 0.0
    ...
    def _get_report_values(self, docids, data=None):
        ...
        # account types por selección (clientes/proveedores/ambos)
        ...
        # ---------------------------------------------
        # >>> NUEVO: respetar partner_ids del wizard <<<
        # ---------------------------------------------
        selected_partner_ids = data['form'].get('partner_ids') or []
        if selected_partner_ids:
            # usar exactamente los partners elegidos
            partners = obj_partner.browse(selected_partner_ids)
            partner_ids = partners.ids
        else:
            # comportamiento original: descubrir partners por movimientos
            params = [
                tuple(data['computed']['move_state']),
                tuple(data['computed']['account_ids']),
            ] + query_get_data[2]
